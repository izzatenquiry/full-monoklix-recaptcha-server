...
/**
 * Verify reCAPTCHA token using Google reCAPTCHA Enterprise
 */
...
app.post('/api/veo/generate-t2v', async (req, res) => {
  ...
  try {
    ...
    if (recaptchaToken) {
      log('log', req, '🔒 reCAPTCHA token provided, adding to header...');
    }
    ...
    // 4. ADD RECAPTCHA TOKEN TO HEADER (if present)
    if (recaptchaToken) {
